			...
			if (newRoleName === "") {
				...
				if (role_permission.length !== 0) {
					...
					const callback = () => {
						...
						this.handleClearPermissionCheckbox();
					}
					...
					<BackLayout>
						<div className='bg'>
							...
							<table className="col-12 admin_table">
								...
								<tbody>
									{(RoleList === undefined || RoleList.list.length === 0) ?
										<tr>
													...
													<tr key={`role${index}`} className={array.includes(`${item.Id}`) ? "onchange" : ""}>
														...
														<td>
															<div className="action">
																<div onClick={(e) => this.handleEditPermission(`${item.Id},${item.Name}`)} className="svg">
									...
									{PermissionList === undefined ? [] : PermissionList.map((item, index) =>
										<div key={`per${index}`} className='t'>
											<input
												...
												name='permission'
												type='checkbox'
												onChange={(e) => { this.handleSelectPermission(e) }}
											/>
											<span>{item.Name}</span>
										</div>
									)}
