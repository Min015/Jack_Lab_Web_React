...
export default connect(mapStateToProps, mapDispatchToProps)(
  class TypeManage extends Component {
    ...
    render() {
      const { ProjectType } = this.props;
      const { table_header, array, add, edit, delO, delAll, newType, now, newName, nowItem } = this.state;
      return (
        <BackLayout>
          <div className="work">
          ...
          <table className="col-12 admin_table">
            ...
            <tbody>
              {(ProjectType === undefined || ProjectType.list.length === 0) ? "" : ProjectType.list.map(
                (item, index) => {
                  return (
                      ...
                      <td>
                        <div className="action">
                          <div onClick={() => this.drop_down('edit')} className="svg">
                            <svg id={`${item.Id},${item.Name}`} onClick={this.handelSetNow.bind()}
                              width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path id={`${item.Id},${item.Name}`} onClick={this.handelSetNow.bind()}
